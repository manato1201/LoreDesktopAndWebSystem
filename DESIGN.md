...
このドキュメントは **Lore Ecosystem 全体(LoreHub Web / LoreForge Client / LoreForge Server Admin)が共有する単一のビジュアルデザインシステム**である。

- **LoreHub (Web)**: Next.js + Tailwind CSS で実装。本ドキュメントのhex値・数値をそのままTailwind theme拡張・CSSカスタムプロパティとして使用する。
- **LoreForge Client / LoreForge Server Admin (Qt 6 / QML)**: 本ドキュメントのトークンを `Theme.qml` 等のシングルトンに移植して使用する。対応関係は「10. Cross-Platform Token Mapping」を参照。

以降の章(1〜9)はWeb実装を前提とした具体的なCSS構文(shadow構文・padding指定等)を含むが、これは**参考実装例**であり、値そのもの(hex・px・weight等)がプラットフォーム共通のトークンである。Qt/QML実装時はCSS構文をそのまま使わず、同等のQML表現(`color:`, `radius:`, `border.width` 等)に読み替えること。

システム全体のコンポーネント構成は [`ARCHITECTURE.md`](./ARCHITECTURE.md)、LoreHubの画面設計は [`LOREHUB_UI_SPEC.md`](./LOREHUB_UI_SPEC.md) を参照。
...
## 10. Cross-Platform Token Mapping

LoreHub(CSS変数/Tailwind)と LoreForge Client / Server Admin(QML `Theme.qml` シングルトン想定)の対応表。値(hex/px/weight)は共通、命名規約のみプラットフォームごとに異なる。

### Colors

| 役割 | Hex | CSS変数(Web) | QMLプロパティ(想定) |
|---|---|---|---|
| Deepest background | `#121212` | `--color-bg-base` | `Theme.colorBackgroundBase` |
| Surface (cards/sidebar) | `#181818` | `--color-bg-surface` | `Theme.colorSurface` |
| Interactive surface | `#1f1f1f` | `--color-bg-interactive` | `Theme.colorSurfaceInteractive` |
| Elevated card | `#252525` | `--color-bg-card-alt` | `Theme.colorSurfaceElevated` |
| Accent (functional only) | `#1ed760` | `--color-accent` | `Theme.colorAccent` |
| Accent border variant | `#1db954` | `--color-accent-border` | `Theme.colorAccentBorder` |
| Text primary | `#ffffff` | `--color-text-base` | `Theme.colorTextPrimary` |
| Text secondary | `#b3b3b3` | `--color-text-muted` | `Theme.colorTextSecondary` |
| Border (default) | `#4d4d4d` | `--color-border` | `Theme.colorBorder` |
| Border (outlined btn) | `#7c7c7c` | `--color-border-light` | `Theme.colorBorderLight` |
| Negative/error | `#f3727f` | `--color-negative` | `Theme.colorNegative` |
| Warning | `#ffa42b` | `--color-warning` | `Theme.colorWarning` |
| Announcement/info | `#539df5` | `--color-announcement` | `Theme.colorAnnouncement` |

### Typography

| 役割 | Size | Weight | CSS変数(Web) | QMLプロパティ(想定) |
|---|---|---|---|---|
| Section Title | 24px | 700 | `--font-size-section-title` | `Theme.fontSizeSectionTitle` (Font.Bold) |
| Feature Heading | 18px | 600 | `--font-size-heading` | `Theme.fontSizeHeading` (Font.DemiBold) |
| Body | 16px | 400 | `--font-size-body` | `Theme.fontSizeBody` (Font.Normal) |
| Button | 14px | 700 | `--font-size-button` | `Theme.fontSizeButton` (Font.Bold) |
| Caption | 14px | 400 | `--font-size-caption` | `Theme.fontSizeCaption` |
| Small | 12px | 400 | `--font-size-small` | `Theme.fontSizeSmall` |

Web: `SpotifyMixUI` / `SpotifyMixUITitle` を利用不可の場合は近似の可変フォント(例: `Inter`)にフォールバックし、フォールバックスタックは維持する。QML: `FontLoader` で同一フォントファイルを読み込み、`Theme.fontFamilyBody` / `Theme.fontFamilyTitle` として公開する。

### Radius, Spacing, Shadow

| 役割 | 値 | CSS変数(Web) | QML実装メモ |
|---|---|---|---|
| Subtle radius | 4px | `--radius-subtle` | `Rectangle.radius: Theme.radiusSubtle` |
| Standard radius | 6px | `--radius-standard` | 同上 |
| Comfortable radius | 8px | `--radius-comfortable` | 同上 |
| Pill (large) | 500px | `--radius-pill` | QMLに`%`指定は無いため `radius: height / 2` を実行時計算 |
| Circle | 50% | `--radius-circle` | `radius: width / 2`（正方形要素前提） |
| Base spacing unit | 8px | `--spacing-unit` | `Theme.spacingUnit: 8` |
| Heavy shadow | `rgba(0,0,0,0.5) 0 8px 24px` | `--shadow-heavy` | `Qt5Compat.GraphicalEffects` の `DropShadow`(または Qt Quick `MultiEffect.shadowBlur`)で同等値を再現 |
| Medium shadow | `rgba(0,0,0,0.3) 0 8px 8px` | `--shadow-medium` | 同上 |

QML/Qt 6にはCSSの`box-shadow`に相当するネイティブプロパティが無いため、`MultiEffect`(Qt 6.5+)または`Qt5Compat.GraphicalEffects`の`DropShadow`要素で近似する。`Theme.qml`実装時にこの表のhex/px値を直接ハードコードせず、シングルトンのプロパティとして一元管理すること。
